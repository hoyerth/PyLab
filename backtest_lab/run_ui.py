# backtest_lab/run_ui.py
"""Zellenuebergreifender Lauf-Status fuer Backtests (Fortschritt + Abbruch).

Analoge Umsetzung zu `signal_lab/sweep_ui.py` (gefixtes Signal-Lab-Pattern,
Schritt 8): Ein Modul-Dict als Single Source of Truth fuer die
Fortschrittsanzeige im Notebook. Ein Hintergrund-Thread aktualisiert den
Status; die UI-Zelle liest ihn ueber `mo.ui.refresh` periodisch neu.
"""
import threading
...
_state: Dict[str, Any] = {
    ...
    "message": "",          # kurze Statusmeldung fuer die UI
    "message_kind": "",     # "spinner" | "ok" | "err" | "info"
    "_thread": None,        # Worker-Thread (fuer is_alive-Check)
}
...
def set_message(msg: str, kind: str = "info") -> None:
    """Setzt eine kurze Statusmeldung (in der Fortschritts-Zelle angezeigt)."""
    with _lock:
...
def reset(total: int) -> None:
    ...
    with _lock:
        _state.update(
            ...
            message="", message_kind="", _thread=None,
        )
...
def set_thread(t) -> None:
    """Merkt den laufenden Worker-Thread (fuer is_alive-Check)."""
    with _lock:
        _state["_thread"] = t
...
def is_alive() -> bool:
    ...
    Bei running=True, aber totem Thread, haengt der Zustand (z. B. nach
    Kernel-Neustart oder Absturz) -> UI kann den Zustand bereinigen.
    """
    with _lock:
...
def finish(ids: List[str], summary: Dict[str, Any] = None) -> None:
    """Schliesst den Lauf ab (normal oder abgebrochen)."""
    with _lock:
