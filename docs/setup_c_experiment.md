...
| 05.09.2026 | **Lücke-1-Integration (Regime-Gate in Produktionskern):** ① `fix` `scripts/regime_filter.py` — Schwellen-Defaults 0.05/−0.02 → versiegelte Siegel-Werte **0.07/−0.03** (§2.16-C Single Source of Truth: `RegimeSchwellen()` ≡ Freeze, verhindert stillen Model Drift); ② `feat` `scripts/setup_c_profil.py` — optionales Gated-Portfolio `--mit-regime-gate` (Default aus = bitgenau Phase-1-Pfad; Portfolio exakt wie OOS-abgenommen §2.16-F.1: TREND → RAW-A@96 + RAW-B@96 nur Bruchrichtung-Fallback, SHAKEOUT/UNKLAR → strikt RAW-A@48); entkoppelter `IRegimeProvider`/`RegimeFilterProvider` (regime_filter-Import nur im Gate-Pfad); Gate-Reports getrennt `reports/setup_c/setup_c_gate_{f}.txt/.tsv` (Baseline-Artefakte byte-identisch); **Verifikation:** Baseline-Lauf `--fenster=ALLE` bitgenau §2.15 (S1 +22,81/+27,87; S2 +0,31/+0,98; AUG +3,64/−0,87; supprimiert=0); Gate-Lauf: AUG Δ+2,16R (RAW-B-Fallback 1 T), S1 Δ+24,15R (52 T/77 S/9 U; RAW-B 16 T +23,13), S2 Δ+22,75R (42 T/15 S/4 U; RAW-B 33 T +22,07); No-Harm-Identität S/U bestätigt; ③ `docs` §4; 3 atomare Commits | abgeschlossen |
| 05.09.2026 | **Lücke 2 (Chart-/Report-Modul `scripts/setup_c_chart.py`):** eigenständiger Baseline-Report — **PNG 300 dpi** (17×11", 3300×5100 px) je (Fenster × Horizont) `setup_c_chart_{AUG,S1,S2}_N{48,96}.png`: Preis high/low, dezente Phasen-Hintergründe, Entry-Marker ^/v mit `P{n} L/S {preis}`-Annotation, Exit-x (SL rot / ZEIT blau / ZENS grau), Verbindungslinie Entry→Exit (grün/rot), CRV-Panel (steps-post, fill_between) + Statistik-Box **oben mittig** (inkl. max. Gewinn-/Verlustserie, max. Drawdown R, PF, WR, Exit-Verteilung, mittl. Haltedauer); **TXT je Fenster** `setup_c_chart_{f}.txt` mit Statistik-Header N48/N96 + allen Trades inkl. `entry_ts`/`exit_ts` (zensierte strikt isoliert, `r_f4=-`); Aufruf `python -m scripts.setup_c_chart --fenster=ALLE --dpi=300` (Namespace-Package; Ausgabe `reports/setup_c/`, unversioniert); **Verifikation:** Syntax-Check OK; Statistik-Sollwerte **bitgenau §2.15** (AUG N48 +3,64R / N96 −0,87R, n=2; S1 +22,81R / +27,87R, n=38; S2 +0,31R / +0,98R, n=3, 1 zensiert; max. Verlustserie AUG 1×−1,00R / S1 4×−3,20R u. 5×−4,40R / S2 1×−1,00R); Suppression-No-op-Assert je Fenster; Baseline unverändert (nur Import `_kern_lauefe`); **Lücke 2 abgenommen**; Commits: ① `feat` `scripts/setup_c_chart.py`, ② `docs` §4 | abgeschlossen |

## §5. Trend Engine Evolution & Audit (Session-Erkenntnisse)

### 5.1 Dekonstruktion des starren Zeithorizonts (Time-Stop N=48/96)
* **Befund (Chart-Audit AUG, 18.–21.08.):** Ein starrer Zeithorizont von 48 bzw. 96 M15-Bars fungiert in echten Trends als algorithmischer Rendite-Dieb. Der Trend am 18./19.08. erstreckte sich über ~2,5 Handelstage (~240 M15-Bars). 
* **Versagen:** Bei $N=48$ wird die Position nach 20 % der Strecke abgewürgt; bei $N=96$ nach 40 %. Das System schaut dem profitabelsten Teil des Runs tatenlos zu.
* **Gewinn-Erosion (P1/P2 in AUG):** Positionen mit massivem zwischenzeitlichem Buchgewinn gaben den Profit über Dutzende Bars wieder vollständig ab, weil der bisherige $F_4$-Trailing-Stop ohne neue Phasenkanten meilenweit zurückblieb.
* **Architektur-Entscheidung:** Der primäre Ausstieg wird vollständig von starren Kerzen-Zählern entkoppelt. Das Trade-Management wechselt auf struktur- und indikatorgeführtes Trailing.

### 5.2 Dynamic Trailing: EMA-20 Steigung (Slope & Curvature)
* **Keine Retail-Cross-Mechanik:** Der simple Schnitt von Preis und gleitendem Durchschnitt ("Price crosses EMA 20") wird strikt verworfen, da er in normalen Konsolidierungen zu verfrühten Whipsaws und Liquiditäts-Abfischungen führt.
* **Ausstiegs-Kriterium:** Auswertung der ersten Ableitung ($\text{Slope}_t = \text{EMA}_t - \text{EMA}_{t-1}$) und deren Abflachung.
* **Ausführungs-Modus (Variante B - Primär):**
  * Sobald eine signifikante Abflachung der EMA-Steigung erkannt wird, erfolgt **kein** panischer Sofort-Verkauf am Open (Vermeidung von Verkäufen in Konsolidierungs-Tiefs).
