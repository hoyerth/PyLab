...
> (1) U 66.45 11.8 03:45–18.8 03:00 / L 64.24 11.8 08:30–13.8 21:15;
...
>
> **Stand (03.09.2026, nach Commit `a2cc7ec`):** **R1_L-PREIS KORRIGIERT
> (User-Tippfehler 62.24 → 64.24).** In `scripts/phasen_volumen_profil.py`
> (`USER_LINES_AUG`): R1_L = **64.24** (11.08 08:30–13.08 21:15) statt 62.24.
> Alle Standard-Artefakte neu erzeugt (AUG-macro-Lauf): .txt-Referenzblock
> und .png-Overlay zeigen 64.24. **Benchmark-Report (Tol 0.15) neu:**
> R1_L 64.24 → beste Distanz P5-L 64.198, d=0.042 (centgenau), aber P5
> startet 14.08 (nach R1_L-Fenster-Ende 13.08 21:15) → **PREIS-ONLY**
> (statt KEIN bei 62.24, d=0.409). Gesamt: **6 VOLL | 2 PREIS-ONLY | 0 KEIN**
> (vorher 6/1/1). Baseline bitgenau **27 Sig/+24.97R**, `--macro-live`
> **25 Sig/+34.87R** unverändert. Doku: `reclaim_makro_persistenz_design.md`
> (Beispiel „tiefe Alt-Zonen“ ohne R1_L-Bezug), `reclaim_signal_loop_design.md`
> (Historie-Zeile), RECLAIM.md-Historie/Update-Log korrigiert.
> **Offen:** Commit + Tag „Reclaim 0.4 vor Makroerweiterung“ + Push.
>
...
- **Letzter Schritt:** **User-Ideallinien vs. Baseline/EdgeStore (AUG, 02.09.)** — Der User definierte 8 manuelle „finale Range-Linien" (4 Ranges, Zeitfenster + Preis, als Massstab): R1 66.45/64.24 (11.–18.8.; R1_L am 03.09. von 62.24 auf 64.24 korrigiert), R2 67.26/65.66 (20.8.), R3 69.90/68.40 (21.–25.8.), R4 69.58/67.65 (25.–27.8.). Diagnose `test/tmp_reclaim_user_ranges.py` + `tmp_reclaim_user_warmup.py` + `tmp_reclaim_baseline_zones.py`:
  - **EdgeStore (AUG-Fenster allein):** Niveaus teils vorhanden, aber „sleeping" (66.397 für 66.45), 62.24 GAR NICHT (Preis nie im Fenster dort; R1_L am 03.09. auf 64.24 korrigiert → im Fenster präsent, P3/P5-Bereich) → Lebenszyklus ungeeignet für „Range-Grenze gilt über Tage, selten getestet".
  - **EdgeStore MIT Vorlauf ab 01.01.2026:** findet ALLE 8 Linien als Niveau (62.266↔62.24 [R1_L-Analyse mit altem Wert; User-Korrektur 03.09.: 64.24], 68.410↔68.40, 67.649↔67.65, d meist <0.1), aber 5/8 als **sleeping** → Kanten-ERKENNUNG (Pivot-Seeds) funktioniert mit Historie, nur der active/sleeping-Lebenszyklus widerspricht der User-Sicht.
  - **BASELINE (12 AUG-Phasen): trifft 7/8 User-Linien als U_final/L_final mit d<0.15** (66.468↔66.45, 67.201↔67.26, 65.603↔65.66, 69.914↔69.90, 68.370↔68.40, 69.555↔69.58, 67.636↔67.65); R1_L 62.24 (Korrektur 03.09.: 64.24) preislich getroffen via P3-L 64.386 (d=0.146) / P5-L 64.198 (d=0.042). **Kern-Befund: Die User-Ideallinien SIND die U_final/L_final-Schnittmengen-Extreme der Baseline-Phasen** — die Baseline hält die Grenze über die Phasendauer (bis 2-Close-Bruch > Grenze+TOL), der EdgeStore schläfert nach erstem 2-Close-Bruch. Die User-Ranges sind KEINE „aktiven Store-Level", sondern Phasen-Grenzen mit seltenen Tests (24h+ zwischen H/L).
...
- **Letzter Schritt:** **Benchmark verankert & verifiziert (User-Freigabe Option 1, 02.09.)** — `UserMacroLine` (frozen/slots), `USER_LINES_AUG` (8 Linien), `BENCHMARK_TOL=0.15`, `BENCHMARK_TOL_EXACT=0.06`, `benchmark_report()` + `if "--benchmark" in sys.argv` **ans Dateiende** von `scripts/phasen_volumen_profil.py` angehängt. Rein lesend (kein Eingriff in Phasen/Signale/Exits/SL). **Verifikation:** `py_compile` ✓; Referenzlauf OHNE Flag exakt **27 Sig / +24.97R / 44 %** ✓ (Null-Einfluss-Beweis); Lauf MIT Flag erzeugt den Report. **Report (AUG, Tol 0.15):**
  ...
  | R1_L 64.24 | 11.08 08:30–13.08 21:15 | P5 L 64.198 | 0.042 (centgenau) | PREIS-ONLY |
  ...
  **Resultat: 6 VOLL | 1 PREIS-ONLY | 1 KEIN.** Die Zeit-Überlappungs-Pflicht (User-Vorgabe) korrigiert die frühere 7/8-Zählung: **R4_L ist streng genommen Post-Hoc** — das beste Niveau 67.636 trägt P12 (26.08 16:45–27.08 19:00), die erst **1 h nach Ende des User-Fensters** (26.08 15:45) startet; einziger In-Fenster-Kandidat in Tol wäre P10-L 67.544 (d=0.106, nur loses Band-Match). R1_L (64.24-Korrektur 03.09.) ist preislich centgenau getroffen (P5-L 64.198, d=0.042), aber P5 startet erst 14.08 (nach R1_L-Fenster-Ende 13.08 21:15) → PREIS-ONLY statt KEIN. → **6/8 User-Linien kausal (Fenster + Preis) reproduziert; R1_L nur preislich (Zeit-Lücke), R4_L wird 1 h zu spät ratifiziert** — exakt der Post-Hoc-Tatbestand, den die Überlappungs-Regel sichtbar machen soll.
...
| 02.09.2026 | Benchmark | **Benchmark-Set verankert (Option 1, User-Freigabe):** `UserMacroLine`/`USER_LINES_AUG` (8 Linien) + `benchmark_report()` ans Dateiende von `scripts/phasen_volumen_profil.py`; Aktivierung `--benchmark`, rein lesend. **Verifiziert:** py_compile ✓; Referenz OHNE Flag exakt 27 Sig/+24.97R/44 % ✓ (Null-Einfluss). **Report: 6 VOLL | 2 PREIS-ONLY | 0 KEIN (Stand 03.09. nach R1_L-Korrektur 62.24→64.24)** — R1_U/R2_U/R2_L/R3_U/R3_L/R4_U VOLL (alle centgenau d≤0.059); **R4_L PREIS-ONLY** (Niveau 67.636 in P12, startet 1 h nach User-Fenster-Ende 26.08 15:45 → Zeit-Überlappungs-Regel entlarvt 7. „Treffer" als Post-Hoc); R1_L PREIS-ONLY (64.24, d=0.042 zu P5-L 64.198, P5 startet 14.08 nach Fenster-Ende 13.08). Damit **6/8 kausal** reproduziert; R4_L-Semantik (streng best-first vs. „beste überlappende Phase in Tol") bei Bedarf als Option dokumentiert. |
...
| 03.09.2026 | R1_L-Korrektur | **User-Tippfehler korrigiert:** `USER_LINES_AUG` R1_L **62.24 → 64.24** (11.08 08:30–13.08 21:15). Code + alle Artefakte (.txt-Referenzblock, .png-Overlay) angepasst. Benchmark-Report: R1_L PREIS-ONLY (P5-L 64.198, d=0.042) → **6 VOLL | 2 PREIS-ONLY | 0 KEIN**. Baseline 27/+24.97R bitgenau; `--macro-live` 25/+34.87R. Doku (RECLAIM.md, reclaim_signal_loop_design.md, reclaim_makro_persistenz_design.md) synchron. |
