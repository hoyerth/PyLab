# -*- coding: utf-8 -*-
"""S1-Monatsauswertung: Baseline-Lauf 2026-02-05..2026-08-28 -> je Monat
...
Je Monat (Signal-Zeit UTC, Gruppierung nach Kalendermonat):
  - test/tmp_S1_monatslauf_YYYY-MM.txt  : Monats-Statistik + Trade-Liste
  - test/tmp_S1_monatslauf_YYYY-MM.png  : HiRes-Zoom (figsize 24x13, dpi 300)
...
sys.argv = ["phasen_volumen_profil.py", "--start=2026-02-05", "--ende=2026-08-28"]
_mod = types.ModuleType("pvp_s1_monate")
_mod.__file__ = str(Path("scripts/phasen_volumen_profil.py").resolve())
sys.modules["pvp_s1_monate"] = _mod
ns = _mod.__dict__
...
tsa = df["ts"].values
print(f"df S1: {len(df)} Bars | Phasen {len(phases)} | Signale {len(sigs)}")

# ---------------------------------------------------------------- Log-Parsing + Mapping
log_txt = io.open("test/tmp_S1_monatslauf_trades.txt", encoding="utf-8").read()
...
assert len(trades) == 201 and abs(s_sum - BASELINE_S1) < 0.10, (len(trades), s_sum)
...
print(f"Mapping Log->Signal: {mm}/201 | Summe R {s_sum:+.2f}R")
...
for m in months:
    ...
    lines = []
    lines.append("=" * 100)
    lines.append(f"S1-MONATSBERICHT {m} | Baseline phasen_volumen_profil.py | "
                 f"Fenster 2026-02-05..2026-08-28 | Gesamt +197.26R")
    lines.append("=" * 100)
    ...
    out_txt = OUT_DIR / f"tmp_S1_monatslauf_{m}.txt"
    out_txt.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def render_monat_png(m: str, grp: list[dict]) -> Path:
    ...
    ax.set_title(f"SILVER M15 | S1-Baseline | Monat {m} | Trades {len(grp)} | "
    ...
    fig.tight_layout()
    out_png = OUT_DIR / f"tmp_S1_monatslauf_{m}.png"
