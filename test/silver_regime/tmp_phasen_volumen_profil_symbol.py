...
for _a in sys.argv[1:]:
    ...
    if _a.startswith("--tol-touch="):
        TOL_TOUCH = float(_a.split("=", 1)[1])
        print(f"==> TOL_TOUCH ueberschrieben: {TOL_TOUCH}")
    if _a.startswith("--density-band="):
        DENSITY_BAND = float(_a.split("=", 1)[1])
        print(f"==> DENSITY_BAND ueberschrieben: {DENSITY_BAND}")
    if _a.startswith("--shift-tol="):
        SHIFT_TOL = float(_a.split("=", 1)[1])
        print(f"==> SHIFT_TOL ueberschrieben: {SHIFT_TOL}")
    if _a.startswith("--min-spread-pct="):
        MIN_SPREAD_PCT = float(_a.split("=", 1)[1])
        print(f"==> MIN_SPREAD_PCT ueberschrieben: {MIN_SPREAD_PCT}")
    if _a.startswith("--min-candles="):
        MIN_CANDLES = int(_a.split("=", 1)[1])
        print(f"==> MIN_CANDLES ueberschrieben: {MIN_CANDLES}")
    if _a.startswith("--min-touches="):
        MIN_TOUCHES = int(_a.split("=", 1)[1])
        print(f"==> MIN_TOUCHES ueberschrieben: {MIN_TOUCHES}")
...
ax.set_title(f"{SYMBOL} M15 {START} - {ENDE} - VOLUME-PROFIL-ZONEN (VA_PCT {VA_PCT:.0f}%) + "
...
def render_standard_chart(
    ...
    ax.set_title(f"{SYMBOL} M15 {start} - {ende} | Fenster {fenster}"
