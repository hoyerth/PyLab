...
DREI MODI (ein Vertrag, drei Instanzen -- Spiegelspez Abschnitt 66.5
und §70):
...
# Spiegelspez Abschnitt 66.5 / §70: EIN Vertrag, DREI Instanzen. Die Sollwerte
# sind zugleich der Fail-Loud-Assert-Katalog -- es gibt bewusst KEINEN
# Schalter "Asserts aus" (Abschnitt 66.5 / E4).
AdapterMode = Literal["V01", "V014", "V015"]
...
if not KONF.k67_override_aktiv:
    ...
    _dsoll = 18.012732 if KONF.g4_aktiv else BENCHMARK_V014_DELTA_R
    ...
    assert [_key(t) for t in sorted(REFERENZ,
                                    key=lambda x: x.bar)] == [(980, 73),
                                                              (1020, 73)]
    _neu_soll = [(903, 67), (980, 67), (981, 73), (1020, 67)]
    if KONF.g4_aktiv:
        _neu_soll = sorted(_neu_soll + [(1002, 77)])
    assert sorted(_key(t) for t in NEU) == _neu_soll
...
def _zeile_kopf() -> str:
    ...
    if KONF.g4_aktiv:
        return (f"ADAPTER v0.15 G4  P9 (848-1020) | Decke K67 mit "
...
def png_03_h2_phasen():
    ...
    ax1.set_title(
        ...
        fontsize=12.5)
    if KONF.k67_override_aktiv:
        _p9z = (("P9-Benchmark v0.15 G4: " if KONF.g4_aktiv
...
def png_04_p9_regime():
    ...
    if KONF.g4_aktiv:
        ax1.set_title(
            ...
            f"Adapter v0.15 G4: P9 mit K67-Override + G4-Boden | "
            f"Niveau-Override {KONF.niveau_override_wert:.4f} / "
            f"Boden K77 deklariert "
            f"{P9_BODEN_RECLAIM.boden_deklariert_literal:.4f} | "
...
log = _Protokoll()
log("=" * 110)
_ADAPTER_VER = ("v0.15 G4" if KONF.g4_aktiv
                else ("v0.14" if KONF.k67_override_aktiv else "v0.1"))
log(f"AUG-SICHTTEST -- PNG-SATZ (Modus {KONF.mode}, "
    f"Adapter {_ADAPTER_VER})")
log("=" * 110)
