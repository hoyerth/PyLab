...
ZWEI MODI (ein Vertrag, zwei Instanzen -- Spiegelspez Abschnitt 66.5):
  ...
  * ``KONFIGURATION_V014`` -- P9 mit K67-Niveau-Override 69.8700.
  ...
  Beide Modi teilen denselben Code und denselben RAM-Patch; umgeschaltet wird
...
Satz (Dateinamen = <ausgabe_praefix> + Nummer + Rolle):
  ...
  02_h1_box      Bars 0..660    H1-Box, 8 Trades (arretiert)
...
from backtest_lab.phasen_regime_adapter import (  # noqa: E402
    ADAPTER_V014, BASELINE_V01_H2_R, BENCHMARK_V014_DELTA_R,
    BENCHMARK_V014_GESAMT_R, BENCHMARK_V014_H2_R, DEFAULT_ADAPTER,
    K67_OVERRIDE_69_87, P9, P12_RESERVE, QUARTETT_V014_BARS,
    Hook2ZielModus, PhasenRegimeAdapter,
)

# ------------------------------------------------- Renderer-Konfiguration
# Spiegelspez Abschnitt 66.5: EIN Vertrag, ZWEI Instanzen. Die Sollwerte sind
# zugleich der Fail-Loud-Assert-Katalog -- es gibt bewusst KEINEN Schalter
# "Asserts aus" (Abschnitt 66.5 / E4).
AdapterMode = Literal["V01", "V014"]
...
def _cli() -> Tuple[RendererKonfiguration, str, str]:
    """Liest Modus und optionales Probe-Praefix (siehe Kopf-Docstring)."""
    import argparse
    ...
    ap.add_argument("--probe-praefix", default=None,
                    help="Schreibt den Satz unter diesem Praefix statt unter "
                         "ausgabe_praefix (nur fuer Gegenproben; das "
...
KONF, PROBE_PRAEFIX, PROTOKOLL_NACH = _cli()
adapter: PhasenRegimeAdapter = (ADAPTER_V014 if KONF.k67_override_aktiv
...
# ------------------------------------------------------- FAIL-LOUD-Asserts
assert len(V0) == 14 and abs(R0 - 40.445143) < 1e-5, (len(V0), R0)
assert len(V1_basis) == 15 and abs(RB - 46.866348) < 1e-5, (len(V1_basis), RB)
assert len(V1) == KONF.ziel_trades_gesamt, (KONF.mode, len(V1))
assert abs(R1 - KONF.ziel_r_gesamt) < 1e-4, (KONF.mode, R1)
assert len(h1_1) == 8 and abs(sum(t.r for t in h1_1) - KONF.ziel_r_h1) < 1e-6
...
if not KONF.k67_override_aktiv:
    ...
    for _b, _soll in ((903, 4.1198), (980, 9.9877), (981, 2.6943),
                      (1020, 3.0032)):
        assert abs(_q[_b].r - _soll) < 1e-4, (_b, _q[_b].r)
    assert abs(sum(t.r for t in _q.values()) - 19.804922) < 1e-4
    assert ((_q[903].kid, _q[980].kid, _q[981].kid, _q[1020].kid)
            == (67, 67, 73, 67))
    assert [_key(t) for t in sorted(REFERENZ,
                                    key=lambda x: x.bar)] == [(980, 73),
                                                              (1020, 73)]
    assert sorted(_key(t) for t in NEU) == [(903, 67), (980, 67),
                                            (981, 73), (1020, 67)]
...
def shade_phases(ax, y0: float, y1: float, x0: int, x1: int,
    ...
    for pid, (bs, be, _u, _l) in PHASEN.items():
        ...
        if pid == aktiv:
            ...
            ax.text((a + b) / 2, y1, f"{pid} AKTIV", fontsize=9, ha="center",
                    va="top", color="#1b6b3a", weight="bold", zorder=11)
        elif pid == "P12":
            ax.axvspan(a, b, color=C_RESERVE, alpha=0.10, zorder=0)
            ax.text((a + b) / 2, y1, f"{pid} RESERVE", fontsize=8.5,
    ...
    for t in trades:
        ...
        if ref:
            ...
            ax.annotate(f"K{t.kid} {t.r:+.2f}R\nbar {t.bar}",
                        (t.bar, t.basis), textcoords="offset points",
                        xytext=(0, 16 if t.richtung == "SHORT" else -25),
...
def time_axis(ax, x0: int, x1: int, ticks: int = 13) -> None:
...
def _zeile_kopf() -> str:
    """Kopfzeile des Statistikblocks (modusabhaengig, Abschnitt 66.5)."""
...
def png_01_gesamt():
    ...
    for (b, _se, px, _ref, _dd) in scan["sweep_sperren"]:
        ax1.plot(b, px, "x", ms=7, color="red", zorder=6, mec="black", mew=0.3)
    ...
    stats_panel(axs, [
        ...
        f"{sum(t.r for t in h1_1):+9.6f} R   <- arretierte Baseline "
        f"(bit-identisch)",
        ...
        f"Preis | Preiswechsel-Linien (*): {len(WECHSEL)} | "
    ...
    out = OUTDIR / f"{PROBE_PRAEFIX or ''}{KONF.ausgabe_praefix}01_gesamt.png"
...
def png_02_h1_box():
    ...
    ax1.set_title(
        f"AUG SICHTTEST 02/05 -- H1 BOX (Bars 0..{X1}) | arretierte Baseline "
        f"| {len(h1_1)} Trades / {sum(t.r for t in h1_1):+.6f} R", fontsize=12.5)
    stats_panel(axs, [
        f"H1 BOX (entry_bar < {box_end}): {len(h1_1)} Trades / "
        f"{sum(t.r for t in h1_1):+.6f} R   <- SOLL arretiert (bit-identisch)",
    ...
    out = OUTDIR / f"{PROBE_PRAEFIX or ''}{KONF.ausgabe_praefix}02_h1_box.png"
    fig.savefig(out, dpi=300)
    plt.close(fig)
    return out
...
def png_03_h2_phasen():
    ...
    ax1.set_title(
        ...
        fontsize=12.5)
    if KONF.k67_override_aktiv:
        _p9z = ("P9-Benchmark v0.14 (Quartett): " + " | ".join(
            f"K{t.kid}@{t.bar} {t.r:+.4f} R ({t.stufe})"
            for t in sorted((x for x in V1 if 848 <= x.bar <= 1020),
                            key=lambda x: x.bar))
            + f" | Summe {P9_BEITRAG:+.4f} R   ||   v0.1-Referenz entfaellt: "
              "K73@980 +2.4119, K73@1020 +3.0093")
    else:
    ...
    stats_panel(axs, [
        ...
        _p9z,
        f"Transition-Park 640-847 (MAKRO, unberuehrt): K1@639, K3@650, "
        f"K45@679, K16@715, K51@760 = 5 / +2.4809 R",
    ] + _zeilen_extra())
    legend(ax1, LEG_BASIS + [
        Line2D([0], [0], color="gray", ls=":", lw=1.0,
               label="Tombstone-Band (R21)"),
    ...
    out = OUTDIR / f"{PROBE_PRAEFIX or ''}{KONF.ausgabe_praefix}03_h2_phasen.png"
    fig.savefig(out, dpi=300)
    plt.close(fig)
    return out
...
def png_04_p9_regime():
    ...
    ax1.text(P9.start_bar + 4, y1, "P9 AKTIV 848-1020", fontsize=10.5,
             va="top", color="#1b6b3a", weight="bold", zorder=11)
    ...
    if KONF.k67_override_aktiv:
        ...
        _z4 = [
            ...
            "Override statt Hook 1 (Abschnitt 66.2): basis_bei(980) = 69.9687 "
            "bzw. 69.9513 wird P9-lokal durch 69.8700 ersetzt -> dist > 0, "
            "K67 handelt DIREKT (STUFE_1_IN_BAR); Hook 1 wird gegenstandslos",
            "Austritt: Bar 1021 zurueck auf basis_bei = 69.9513, ab Bar 1022 "
            "69.9458 (alle 5 Wicks) | Grenzlinien 848 / 1021 (Regel 15)",
            "Quartett: " + " | ".join(
                f"K{t.kid}@{t.bar} {t.stufe} entry {t.entry:.4f} "
                f"sl {t.sl:.4f}" for t in _quart),
            "Quartett R: " + " | ".join(
                f"K{t.kid}@{t.bar} {t.r:+.4f}" for t in _quart),
            f"SUMME Quartett-Beitrag: {P9_BEITRAG:+.6f} R | v0.1-Referenz "
            f"K73@980 +2.4119 + K73@1020 +3.0093 = +5.4212 R (entfaellt) | "
            f"K59@853 entfaellt",
        ]
    else:
        _z4 = [
    ...
    fig.subplots_adjust(left=0.045, right=0.995, top=0.955, bottom=0.07)
    out = OUTDIR / f"{PROBE_PRAEFIX or ''}{KONF.ausgabe_praefix}04_p9_regime.png"
    fig.savefig(out, dpi=300)
    plt.close(fig)
    return out
...
def png_05_kantenkarte():
    ...
    fig.subplots_adjust(left=0.04, right=0.995, top=0.955, bottom=0.065)
    out = OUTDIR / f"{PROBE_PRAEFIX or ''}{KONF.ausgabe_praefix}05_kantenkarte.png"
    fig.savefig(out, dpi=300)
    plt.close(fig)
    return out
...
if KONF.k67_override_aktiv:
    log(f"  Quartett       : " + " | ".join(
        f"K{t.kid}@{t.bar} {t.r:+.4f}" for t in sorted(
            ...
            key=lambda x: x.bar)) + f" | Summe {P9_BEITRAG:+.6f} R")
...
log(f"  Preiswechsel-Linien (Label 'v0 -> v1 *'): {len(WECHSEL)}")
...
_PFAD = ROOT / (PROTOKOLL_NACH or KONF.protokoll_datei)
